...
/**
 * The extension class that serves as the integration point between
 * Elasticsearch, X-Pack, and the custom realm that is provided by this
 * extension.
 */
...
* This is necessary to ensure the headers are copied from the incoming request
* and made available to your realm(s).
*/
@Override
public Collection<String> getRestHeaders() {
return Arrays.asList(CustomRealm.DKP_HEADER, CustomRealm.TOKEN_HEADER);
}
/**
* Returns a map of the custom realms provided by this extension. The first
* parameter is the string representation of the realm type; this is the value
* that is specified when declaring a realm in the settings. Note, the realm
* type cannot be one of the types defined by X-Pack. In order to avoid a
* conflict, you may wish to use some prefix to your realm types.
*
* The second parameter is an instance of the {@link Factory} implementation.
* This factory class will be used to create realms of this type that are
* defined in the elasticsearch settings.
*/
public Map<String, Factory> getRealms(ResourceWatcherService resourceWatcherService) {
return new MapBuilder<String, Factory>().put(CustomRealm.TYPE, new CustomRealmFactory())
