...
/*
* The type of the realm. This is defined as a static final variable to prevent
* typos
*/
private static final Logger LOGGER = LogManager.getLogger("accessLogger");
public static final String TYPE = "shieldrealm";
public static final String DKP_HEADER = "X-Logon-User";
public static final String TOKEN_HEADER = "Token";
private final Map<String, InfoHolder> usersMap;
/**
* Constructor for the Realm. This constructor delegates to the super class to
* initialize the common aspects such as the logger.
* 
* @param config
...
/**
* This constructor should be used by extending classes so that they can specify
...
* possible. This implementation of token extraction looks for two headers, the
* <code>User</code> header for the username and the <code>Password</code>
* header for the plaintext password
* 
* @param threadContext
*            the {@link ThreadContext} that contains headers and transient
...
String user = threadContext.getHeader(DKP_HEADER).substring("DKP\\".length()).trim();
LOGGER.info("Taking DKP\\" + user + " from HTTP header request");
if (user != null) {
String password = null;
try {
password = Base64.decode(threadContext.getHeader(TOKEN_HEADER).substring("TOKEN\\".length()).trim()).toString();
LOGGER.info("Taking DKP\\" + user + " with private token " + password + " from HTTP header request");
} catch (ParseException e) {
LOGGER.error("ParseException");
throw new IllegalArgumentException(e);
}
if (password != null) {
LOGGER.info("New UsernamePasswordToken generated for DKP\\" + user + " with private token " + password);
...
/**
* Method that handles the actual authentication of the token. This method will
* only be called if the token is a supported token. The method validates the
* credentials of the user and if they match, a {@link User} will be returned as
* the argument to the {@code listener}'s
...
* authentication is performed by this method. If this realm does not support
* user lookup, then this method will not be called.
* 
* @param username
*            the identifier for the user
* @param listener
*            used to return lookup result
*/
@Override
